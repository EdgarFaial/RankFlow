...
const App: React.FC = () => {
  ...
  useEffect(() => {
    ...
    if (customTheme) {
      localStorage.setItem('rankflow_custom_theme', JSON.stringify(customTheme));
    }
  ...
  const moveTask = useCallback((taskId: string, direction: 'up' | 'down') => {
    setTasks(prev => {
      ...
      return prev.map(t => {
        if (t.id === taskA.id) return taskA;
        if (t.id === taskB.id) return taskB;
        return t;
      });
    });
  }, [activeCriterion]);
  ...
  const statsData = useMemo(() => 
    RANKING_CONFIGS.map(c => ({
      ...
      avg: tasks.length > 0 ? tasks.reduce((a, b) => a + (b[c.id] || 0), 0) / tasks.length : 0
    })), 
    [tasks]
  );
  ...
  const statusDist = useMemo(() => [
    ...
    { name: 'Feito', value: tasks.filter(t => t.status === TaskStatus.DONE).length, fill: '#10b981' }
  ], [tasks]);
    ...
    <div className={`flex flex-col md:flex-row h-screen overflow-hidden ${theme.mainBg} transition-all duration-300 relative`}>
      ...
      <aside className={`hidden md:flex flex-col w-72 ${theme.sidebarBg} text-white p-8 shrink-0 shadow-2xl z-20`}>
        <div className="flex items-center gap-3 mb-12">
          <div className={`${theme.accent} p-2.5 rounded-2xl shadow-lg`}>
            <Sparkles size={28} />
          </div>
        ...
        {aiAudit && view === 'tasks' && (
          <div className="mx-8 mt-6 p-6 rounded-3xl bg-indigo-600 text-white shadow-2xl animate-in slide-in-from-top-4 duration-500 relative overflow-hidden">
            ...
            <button 
              ...
              className="absolute top-4 right-4 hover:bg-white/20 p-1 rounded-full"
            >
              <X size={20}/>
            </button>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
              {aiAudit.suggestions?.map((s, i) => (
                <div 
                  key={i} 
                  className="bg-white/10 backdrop-blur-sm p-4 rounded-2xl border border-white/20 flex gap-3 items-start"
                >
                ...
                {sortedTasks.length === 0 ? (
                  <div className={`flex flex-col items-center justify-center py-32 text-slate-400 border-4 border-dashed ${theme.border} rounded-[3rem]`}>
                    ...
                    <p className="text-xl font-bold italic">
                      Sua lista está vazia. Comece a criar!
                    </p>
                  </div>
                ) : (
              ...
              <div className="grid grid-cols-1 gap-5">
                {habits.length === 0 ? (
                  <div className={`flex flex-col items-center justify-center py-32 text-slate-400 border-4 border-dashed ${theme.border} rounded-[3rem]`}>
                    ...
                    <p className="text-xl font-bold italic">
                      Nenhum hábito rastreado. Mude sua rotina!
                    </p>
                  </div>
                ) : (
                  habits.map(habit => (
            ...
            <div className="max-w-6xl mx-auto space-y-10">
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
                {[
                  { 
                    ...
                    val: tasks.filter(t => t.status !== TaskStatus.DONE).length, 
                  ...
                  { 
                    label: 'Consistência', 
                    val: habits.length > 0 
                      ? `${Math.min(100, Math.round((habits.reduce((a, b) => a + b.completedDates.length, 0) / 30) * 100))}%` 
                      : '0%', 
                  ...
                  >
                    ...
                    <p className={`text-xs font-black uppercase tracking-widest ${theme.textSecondary} mt-2`}>
                      {stat.label}
                    </p>
                  </div>
                ))}
              ...
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
                <div className={`${theme.cardBg} p-10 rounded-[3rem] border ${theme.border} h-[400px] shadow-xl`}>
                  <h3 className={`text-lg font-black uppercase tracking-widest ${theme.textPrimary} mb-8`}>
            ...
            <div className="max-w-2xl mx-auto space-y-10">
              <div className={`${theme.cardBg} border ${theme.border} rounded-[3rem] overflow-hidden shadow-2xl backdrop-blur-md`}>
                <div className={`p-10 border-b ${theme.border} flex items-center justify-between`}>
                  <div>
                    <h3 className={`text-2xl font-black ${theme.textPrimary} tracking-tighter`}>
                      Aparência
                    </h3>
                    <p className={`text-sm ${theme.textSecondary} font-medium`}>
                      Molde o RankFlow ao seu estilo.
                    </p>
                  </div>
                  <button 
                    onClick={() => setThemeMode(themeMode === 'light' ? 'dark' : 'light')} 
                    className={`p-4 rounded-[1.5rem] shadow-inner transition-all ${
                      themeMode === 'light' 
                ...
                <div className="p-10 space-y-10">
                  <div className="grid grid-cols-2 gap-4">
                    {themesList.map(t => (
                      <button 
                        ...
                        className={`p-6 rounded-[2rem] border-4 transition-all flex flex-col gap-4 items-center group ${
                          themeName === t.id 
                  ...
                  <div className="space-y-4">
                    <label className={`block text-xs font-black uppercase tracking-[0.2em] ${theme.textSecondary}`}>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 backdrop-blur-xl bg-black/40">
          <div className={`relative ${theme.cardBg} w-full max-w-xl rounded-[3rem] shadow-2xl overflow-hidden animate-in zoom-in duration-300`}>
            <div className={`p-10 border-b ${theme.border} flex justify-between items-center bg-gradient-to-r from-indigo-500/10 to-transparent`}>
              <h3 className={`text-3xl font-black ${theme.textPrimary} tracking-tighter`}>
                Nova Tarefa
              </h3>
              <button 
            ...
            <div className="p-10 space-y-6">
              <input 
                ...
                className={`w-full p-6 rounded-3xl border ${theme.border} ${theme.inputBg} ${theme.textPrimary} text-lg font-bold outline-none`} 
              />
              ...
              <textarea 
                ...
                className={`w-full p-6 rounded-3xl border ${theme.border} ${theme.inputBg} ${theme.textPrimary} h-40 resize-none font-medium`} 
              />
              
              <div className="flex flex-col gap-2">
                <label className="text-[10px] font-black uppercase tracking-widest opacity-50 ml-2">
                ...
                <input 
                  ...
                  className={`w-full p-6 rounded-3xl border ${theme.border} ${theme.inputBg} ${theme.textPrimary} font-bold`} 
                />
              </div>
              ...
              <div className="flex items-center gap-3 p-4 bg-indigo-500/5 rounded-3xl border border-indigo-500/10">
                <input 
                  type="checkbox" 
                  id="ai" 
                  checked={useAiForThisTask} 
                  onChange={e => setUseAiForThisTask(e.target.checked)} 
                  className="w-5 h-5 accent-indigo-500" 
                />
                <label 
                  htmlFor="ai" 
                  className="text-sm font-black uppercase text-indigo-500 cursor-pointer"
                >
                  Usar IA para detalhar tarefa
                </label>
              </div>
            </div>
            ...
            <div className="p-10 bg-slate-50 dark:bg-black/20 flex gap-4">
              ...
              <button 
                onClick={addTask} 
                className={`flex-[2] p-6 ${theme.accent} text-white font-black uppercase text-xs tracking-[0.2em] rounded-[2rem] shadow-xl hover:scale-105 active:scale-95 transition-all`}
              >
                CRIAÇÃO IMEDIATA
              </button>
            </div>
          ...
          <div className={`relative ${theme.cardBg} w-full max-w-xl rounded-[3rem] shadow-2xl overflow-hidden animate-in zoom-in duration-300`}>
            ...
            <div className="p-10 space-y-8">
              <div className="space-y-3">
                <label className={`block text-xs font-black uppercase tracking-widest ${theme.textSecondary} ml-2`}>
